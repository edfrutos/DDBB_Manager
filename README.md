...
El arranque está encapsulado en helpers testeables en `main_gui.py` (`build_application()` y `schedule_autoconnect()`).
El flujo de conexión está cubierto por `ConnectionDialog`, `initialize_connection()` y `open_connection_dialog()`.
