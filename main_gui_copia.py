...
# Importar MainWindow después de asegurar que las bibliotecas están disponibles
...
def main():
    ...
    # Configurar manejadores de señales antes de iniciar la aplicación
    manejar_senales()
    ...
    if mongodb_uri:
        ...
        def conexion_retrasada():
            try:
                # Intentar conexión inicial usando el método adecuado
                if hasattr(main_window, 'connect_to_mongodb'):
                    main_window.connect_to_mongodb()
                else:
                    # Fallback a initialize_connection si está disponible
